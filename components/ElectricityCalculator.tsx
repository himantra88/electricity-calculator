...
import { 
  ...
  Home, Building2, TrendingUp, DollarSign, ListOrdered, FileSearch, 
  TableProperties, ArrowRight, Calculator, Search, Plus, Minus, 
  Wind, Aperture, Snowflake, Tv, Thermometer, Disc, Coffee, Shirt, Laptop
} from 'lucide-react';
...
// --- INDIAN HOME APPLIANCE LIBRARY ---
const appliancesList = [
  { id: 'ac15', name: '1.5 Ton Split AC', watts: 1500, icon: Snowflake },
  { id: 'cooler', name: 'Desert Cooler', watts: 250, icon: Wind },
  { id: 'bldcfan', name: 'BLDC Ceiling Fan', watts: 28, icon: Aperture },
  { id: 'oldfan', name: 'Standard Fan', watts: 75, icon: Aperture },
  { id: 'fridge_s', name: 'Fridge (Single Door)', watts: 150, icon: Snowflake },
  { id: 'fridge_d', name: 'Fridge (Double Door)', watts: 250, icon: Snowflake },
  { id: 'geyser', name: 'Water Heater/Geyser', watts: 2000, icon: Thermometer },
  { id: 'wm', name: 'Washing Machine', watts: 500, icon: Disc },
  { id: 'mixer', name: 'Mixer Grinder', watts: 500, icon: Coffee },
  { id: 'iron', name: 'Electric Iron', watts: 1000, icon: Shirt },
  { id: 'tv', name: 'LED TV 43"', watts: 60, icon: Tv },
  { id: 'laptop', name: 'Laptop/PC', watts: 100, icon: Laptop },
  { id: 'led', name: 'LED Bulb', watts: 9, icon: Lightbulb },
  { id: 'tube', name: 'Tube Light', watts: 20, icon: Lightbulb },
];

// --- CLAYMORPHISM STYLES ---
const clayBg = "bg-[#F2F5F9]";
const clayText = "text-[#3D405B]";
const clayAccent = "text-[#6C5DD3]";
const clayCard = "bg-[#F2F5F9] rounded-[2.5rem] shadow-[10px_10px_20px_#d1d5df,-10px_-10px_20px_#ffffff,inset_3px_3px_5px_rgba(255,255,255,0.8),inset_-3px_-3px_5px_rgba(0,0,0,0.03)] border-2 border-white/60";
const clayCardPrimary = "bg-[#6C5DD3] rounded-[2.5rem] shadow-[10px_10px_20px_#d1d5df,-10px_-10px_20px_#ffffff,inset_3px_3px_5px_rgba(255,255,255,0.3),inset_-3px_-3px_5px_rgba(0,0,0,0.1)] text-white border-2 border-[#8174E0]";
const clayInput = "bg-[#F2F5F9] rounded-[1.5rem] shadow-[inset_5px_5px_10px_#d1d5df,inset_-5px_-5px_10px_#ffffff] border-none outline-none focus:ring-2 focus:ring-[#6C5DD3] focus:bg-white transition-all";
const clayBtn = "bg-[#6C5DD3] text-white rounded-[1.5rem] shadow-[6px_6px_12px_#d1d5df,-6px_-6px_12px_#ffffff,inset_3px_3px_6px_rgba(255,255,255,0.3),inset_-3px_-3px_6px_rgba(0,0,0,0.15)] active:shadow-[inset_4px_4px_8px_rgba(0,0,0,0.2),inset_-4px_-4px_8px_rgba(255,255,255,0.2)] active:scale-[0.98] transition-all font-bold border border-[#8174E0]";
const clayBtnSecondary = "bg-[#F2F5F9] text-[#6C5DD3] rounded-[1rem] shadow-[5px_5px_10px_#d1d5df,-5px_-5px_10px_#ffffff,inset_2px_2px_4px_rgba(255,255,255,1),inset_-2px_-2px_4px_rgba(0,0,0,0.03)] active:shadow-[inset_5px_5px_10px_#d1d5df,inset_-5px_-5px_10px_#ffffff] active:scale-[0.98] transition-all font-bold border border-white";
const clayIcon = "bg-[#F2F5F9] rounded-[1.2rem] shadow-[5px_5px_10px_#d1d5df,-5px_-5px_10px_#ffffff,inset_2px_2px_4px_rgba(255,255,255,1),inset_-2px_-2px_4px_rgba(0,0,0,0.03)] p-3 flex items-center justify-center border border-white";
...
export default function ElectricityCalculator({ 
  ...
  const [selectedAppliances, setSelectedAppliances] = useState<Record<string, { qty: number, hours: number }>>({});
  ...
  const handleApplianceChange = (id: string, field: 'qty' | 'hours', value: number) => {
    setSelectedAppliances(prev => {
      const current = prev[id] || { qty: 0, hours: 0 };
      ...
      // Default to 1 hour if qty is added but hours were 0
      if (field === 'qty' && updated.hours === 0 && value > 0) {
        updated.hours = 1;
      }
  ...
  const getEstimatedUnits = () => {
    let totalWattsHours = 0;
    Object.entries(selectedAppliances).forEach(([id, data]) => {
      const app = appliancesList.find(a => a.id === id);
      if (app) {
        totalWattsHours += (app.watts * data.qty * data.hours);
      }
    });
    // Calculate total kWh for 30 days
    return ((totalWattsHours / 1000) * 30).toFixed(1);
  };

  const filteredAppliances = appliancesList.filter(app => 
  ...
  return (
    <div className={`min-h-screen ${clayBg} font-sans ${clayText} selection:bg-[#6C5DD3] selection:text-white pb-20 overflow-x-hidden`}>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 space-y-12 mt-12">
        ...
        {billResult && (
          <div className={`${clayCardPrimary} p-10 relative overflow-hidden animate-in fade-in zoom-in duration-500`}>
             ...
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center relative z-10 gap-8">
               ...
               <div className="relative w-36 h-36 flex items-center justify-center shrink-0 bg-[#594CBD] rounded-full shadow-[inset_6px_6px_12px_rgba(0,0,0,0.2),inset_-6px_-6px_12px_rgba(255,255,255,0.1)] border-4 border-[#8174E0]">
        ...
        <section id="calculator" className={`${clayCard} p-8 sm:p-10`}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <button 
              ...
              className={`${clayBtnSecondary} px-5 py-3 flex items-center gap-2 text-sm border-none`}
            >
              <Calculator className="h-4 w-4" /> Estimate via Appliances
            </button>
          </div>
          ...
          {/* THE INDIAN HOME APPLIANCE LIBRARY (Togglable) */}
          {showEstimator && (
            <div className={`mb-10 p-6 sm:p-8 ${clayCard} !bg-[#E5E9F2] !shadow-[inset_5px_5px_12px_#d1d5df,inset_-5px_-5px_12px_#ffffff] !border-none animate-in slide-in-from-top-4`}>
              <div className="flex justify-between items-center mb-6">
                <h4 className="font-black text-xl flex items-center gap-3"><div className={`${clayIcon} !h-10 !w-10 !p-2`}><Home className="h-5 w-5 text-[#6C5DD3]" /></div> Appliance Estimator</h4>
                <button onClick={() => setShowEstimator(false)} className={`${clayBtnSecondary} !p-2`}><X className="h-5 w-5"/></button>
              </div>
              ...
              <div className="mb-6 relative">
                ...
                <input 
                  ...
                  className={`w-full pl-14 pr-5 py-4 font-bold ${clayInput}`}
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-h-96 overflow-y-auto pr-2 pb-2">
                {filteredAppliances.map(app => {
                  const Icon = app.icon;
                  const qty = selectedAppliances[app.id]?.qty || 0;
                  const hours = selectedAppliances[app.id]?.hours || 0;
                  return (
                    <div key={app.id} className={`${clayCard} p-4 !rounded-[1.5rem] flex flex-col sm:flex-row items-center gap-4`}>
                      <div className={`${clayIcon} h-12 w-12 shrink-0`}>
                        <Icon className="h-6 w-6 text-[#6C5DD3]" />
                      </div>
                      <div className="flex-1 text-center sm:text-left">
                        <h5 className="font-extrabold text-[#3D405B]">{app.name}</h5>
                        <p className="text-xs text-gray-500 font-bold">{app.watts}W</p>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col items-center">
                          <span className="text-[10px] font-bold text-gray-400 uppercase mb-1">Qty</span>
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => handleApplianceChange(app.id, 'qty', qty - 1)} className={`${clayBtnSecondary} !p-1.5 !rounded-lg`}><Minus className="h-3 w-3"/></button>
                            <span className="font-black w-4 text-center">{qty}</span>
                            <button onClick={() => handleApplianceChange(app.id, 'qty', qty + 1)} className={`${clayBtnSecondary} !p-1.5 !rounded-lg`}><Plus className="h-3 w-3"/></button>
                          </div>
                        </div>
                        <div className="flex flex-col items-center">
                          <span className="text-[10px] font-bold text-gray-400 uppercase mb-1">Hrs/Day</span>
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => handleApplianceChange(app.id, 'hours', Math.max(0, hours - 1))} className={`${clayBtnSecondary} !p-1.5 !rounded-lg`}><Minus className="h-3 w-3"/></button>
                            <span className="font-black w-4 text-center">{hours}</span>
                            <button onClick={() => handleApplianceChange(app.id, 'hours', Math.min(24, hours + 1))} className={`${clayBtnSecondary} !p-1.5 !rounded-lg`}><Plus className="h-3 w-3"/></button>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="mt-8 pt-6 border-t-2 border-dashed border-gray-300 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="text-center sm:text-left">
                  <p className="text-sm font-bold text-gray-500 mb-1">Estimated Monthly Consumption</p>
                  <p className="text-4xl font-black text-[#6C5DD3]">{getEstimatedUnits()} <span className="text-xl">kWh</span></p>
                </div>
                <button 
                  onClick={() => { setUnits(getEstimatedUnits()); setShowEstimator(false); }} 
                  className={`${clayBtn} px-8 py-4 text-lg w-full sm:w-auto`}
                >
                  Use Estimate
                </button>
              </div>
        ...
        {billResult && (
          <div id="breakdown" className={`${clayCard} p-8 sm:p-10 animate-in slide-in-from-bottom-4`}>
             ...
             <div className="space-y-6">
               <div className={`p-6 ${clayInput} !shadow-[inset_3px_3px_6px_#d1d5df,inset_-3px_-3px_6px_#ffffff] !rounded-[2rem]`}>
               ...
               <div className={`p-6 ${clayInput} !shadow-[inset_3px_3px_6px_#d1d5df,inset_-3px_-3px_6px_#ffffff] !rounded-[2rem] space-y-4 text-base`}>
            ...
            <article className={`${clayCard} p-8`}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className={`w-full h-4 ${clayInput}`}>
                    <div className="bg-gray-400 h-full rounded-full shadow-[inset_2px_2px_4px_rgba(0,0,0,0.2),inset_-2px_-2px_4px_rgba(255,255,255,0.3)]" style={{ width: `${Math.min((compB500 / Math.max(compA500, compB500)) * 100, 100)}%` }}></div>
                  </div>
                </div>
              </div>
            </article>
            ...
            <article className={`${clayCard} p-8 flex flex-col justify-between`}>
              ...
              <div className={`flex items-end justify-around gap-6 h-40 ${clayInput} p-4 !rounded-[2rem]`}>
                ...
                <div className="w-1/2 flex flex-col items-center justify-end gap-3 h-full">
                  ...
                  <div className="bg-gray-400 w-16 rounded-[1.5rem] rounded-b-none shadow-[inset_2px_2px_4px_rgba(0,0,0,0.2),inset_-2px_-2px_4px_rgba(255,255,255,0.3)]" style={{ height: `${(nationalAvg500 / 5000) * 100}%` }}></div>
        ...
        <div className="pt-8">
          ...
          <div className="space-y-8">
            ...
            <article className={`${clayCard} p-8`}>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-8">
                <div className="flex items-center gap-5">
                  <div className={`${clayIcon} h-16 w-16`}><FileSearch className={`h-8 w-8 ${clayAccent}`} /></div>
                  <div>
                    <h4 className="font-black text-2xl">State Tariff Lookup</h4>
                    <p className="text-gray-500 font-bold text-sm">View complete unit slab structures for any state.</p>
                  </div>
                </div>
                <div className="w-full sm:w-72 shrink-0">
                  <ClaySelect value={lookupState} onChange={(e)=>setLookupState(e.target.value)} options={statesList} />
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className={`${clayInput} p-6 !rounded-[1.5rem]`}>
                  <h4 className={`font-black text-xl mb-5 border-b-2 border-gray-200 pb-3 flex items-center gap-3 ${clayAccent}`}><Home className="h-5 w-5"/> Domestic Slabs</h4>
                  <ul className="space-y-4 text-base text-gray-600 font-bold">
                    {tariffData[lookupState]?.Domestic?.slabs?.map((s, i) => (
                      <li key={i} className="flex justify-between border-b border-dashed border-gray-300 pb-2">
                        <span>Up to {s.max === Infinity ? 'Above' : s.max}</span><span className="font-black text-xl text-[#2D3142]">₹{s.rate.toFixed(2)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className={`${clayInput} p-6 !rounded-[1.5rem]`}>
                  <h4 className="font-black text-xl text-gray-600 mb-5 border-b-2 border-gray-200 pb-3 flex items-center gap-3"><Building2 className="h-5 w-5"/> Commercial Slabs</h4>
                  <ul className="space-y-4 text-base text-gray-600 font-bold">
        ...
        <section id="guide" className="space-y-12 pt-8">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <h2 className="text-4xl font-black mb-5 tracking-tight">Understanding Your Bill</h2>
            <p className="text-gray-500 font-bold text-lg">Demystifying the charges, surcharges, and terminology used by Indian electricity distribution boards.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
             <article className={`${clayCard} p-10 hover:scale-[1.01] transition-transform duration-300`}>
               <h3 className="text-2xl font-black mb-6 flex items-center gap-4">
                 <div className={`${clayIcon}`}><Info className="text-[#26C6DA] h-6 w-6"/></div> What is a Billing Slab?
               </h3>
               <p className="text-gray-600 leading-relaxed text-base font-bold">
             ...
             <article className={`${clayCard} p-10 hover:scale-[1.01] transition-transform duration-300`}>
               <h3 className="text-2xl font-black mb-6 flex items-center gap-4">
                 <div className={`${clayIcon}`}><Info className="text-[#FF6188] h-6 w-6"/></div> Res vs Commercial
               </h3>
               <div className="text-gray-600 leading-relaxed text-base font-bold space-y-5">
                 <div className="flex items-start gap-4">
                   <div className={`${clayIcon} !p-2 mt-1 shrink-0`}><Home className={`h-4 w-4 ${clayAccent}`}/></div>
                   <p><strong>Domestic:</strong> Heavily subsidized by the government for residential living to ensure basic access to power.</p>
                 </div>
                 <div className="flex items-start gap-4">
                   <div className={`${clayIcon} !p-2 mt-1 shrink-0`}><Building2 className={`h-4 w-4 ${clayAccent}`}/></div>
                   <p><strong>Commercial:</strong> Charged at higher rates. This extra revenue is used to fund the subsidies provided to domestic and agricultural consumers.</p>
                 </div>
               </div>
             </article>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <article className={`${clayCard} p-10 hover:scale-[1.02] transition-transform duration-300 group`}>
              <div className={`${clayIcon} w-16 h-16 mb-8 group-hover:shadow-[inset_4px_4px_8px_#d1d5df,inset_-4px_-4px_8px_#ffffff] transition-shadow`}>
                <DollarSign className={`h-8 w-8 ${clayAccent}`} />
              </div>
              <h4 className="font-black text-xl mb-4">Fixed Charges</h4>
              <p className="text-sm text-gray-500 leading-relaxed font-bold">A mandatory monthly fee regardless of consumption. It covers the cost of maintaining the power grid, transformers, and the physical wires connecting to your meter. Usually scales with your "Connected Load".</p>
            </article>
            <article className={`${clayCard} p-10 hover:scale-[1.02] transition-transform duration-300 group`}>
              <div className={`${clayIcon} w-16 h-16 mb-8 group-hover:shadow-[inset_4px_4px_8px_#d1d5df,inset_-4px_-4px_8px_#ffffff] transition-shadow`}>
                <TrendingUp className={`h-8 w-8 ${clayAccent}`} />
              </div>
              <h4 className="font-black text-xl mb-4">FPPCA / FAC</h4>
              <p className="text-sm text-gray-500 leading-relaxed font-bold">Fuel and Power Purchase Cost Adjustment. Since coal and gas prices fluctuate in the global market, DISCOMs use this dynamic surcharge per unit to recover unexpected fuel costs without altering base tariffs.</p>
            </article>
            <article className={`${clayCard} p-10 hover:scale-[1.02] transition-transform duration-300 group`}>
              <div className={`${clayIcon} w-16 h-16 mb-8 group-hover:shadow-[inset_4px_4px_8px_#d1d5df,inset_-4px_-4px_8px_#ffffff] transition-shadow`}>
                <ShieldCheck className={`h-8 w-8 ${clayAccent}`} />
              </div>
              <h4 className="font-black text-xl mb-4">Regulatory Surcharges</h4>
              <p className="text-sm text-gray-500 leading-relaxed font-bold">An additional fee sometimes approved by the State Electricity Regulatory Commission (SERC). It allows power companies to recover past financial deficits or fund major state-wide infrastructure upgrades.</p>
            </article>
          </div>
        </section>

        <section id="tips" className={`${clayInput} p-8 md:p-16 !rounded-[3rem] mt-12`}>
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 mb-14 text-center md:text-left">
            <div className={`${clayIcon} !h-20 !w-20`}>
              <Lightbulb className="h-10 w-10 text-[#FFD166] fill-[#FFD166]" />
            </div>
            <h2 className="text-3xl md:text-4xl font-black tracking-tight">10 Proven Energy Saving Tips</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {energyTips.map((tip, idx) => (
              <article key={idx} className={`${clayCard} p-6 !rounded-[1.5rem] flex gap-5 items-start group`}>
                <span className={`${clayIcon} !h-10 !w-10 !rounded-xl font-black text-[#FFD166] shrink-0 text-lg`}>{idx + 1}</span>
                <p className="text-base text-gray-600 font-bold leading-relaxed pt-1.5">{tip}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        {/* FAQs */}
        <div className="pt-12">
          <h2 className="text-3xl font-black mb-8 flex items-center">Learning <span className={`text-sm ${clayBtnSecondary} px-3 py-1 ml-4`}>FAQs</span></h2>
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className={`${clayIcon} shrink-0 h-14 w-14`}>
                      <HelpCircle className={`h-6 w-6 transition-colors ${openFaq === idx ? 'text-[#FFD166] fill-[#FFD166]' : 'text-gray-400'}`} />
                    </div>
                  ...
                  <div className={`relative w-12 h-12 rounded-[1rem] flex items-center justify-center shrink-0 transition-all ${openFaq === idx ? clayInput : clayIcon}`}>
                    <ChevronDown className={`h-6 w-6 transition-transform duration-300 ${openFaq === idx ? 'rotate-180 text-[#6C5DD3]' : 'text-gray-400'}`} />
                  </div>
                </button>
      ...
      {/* CLAY FOOTER */}
      <footer className="mt-24 pt-20 pb-12 bg-white/40 border-t-2 border-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {statesList.map(s => (
                <Link 
                  ...
                  className={`${clayBtnSecondary} flex items-center justify-center h-[40px] w-full text-xs sm:text-sm text-gray-500 font-black px-4 hover:text-[#6C5DD3] text-center border-none`}
                >
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 text-[15px] text-gray-500 font-bold px-4">
            <div className="flex items-center gap-4">
              ...
              <span className="font-black tracking-widest uppercase text-xl text-[#2D3142]">VidyutCalc</span>
            </div>
